...
- Preferred: [GitHub private vulnerability reporting](https://github.com/ullbergm/faa-drone-trainer/security/advisories/new)
...
[faa-drone.ullberg.io](https://faa-drone.ullberg.io), and the service worker
prompts open tabs to reload onto it after each deploy. There are no
maintained older branches.

## Verifying a release

Each release carries `faa-drone-trainer-VERSION.tar.gz`, a copy of exactly the
...
```
gh attestation verify faa-drone-trainer-VERSION.tar.gz \
  --repo ullbergm/faa-drone-trainer
```

Add `--bundle faa-drone-trainer-VERSION.tar.gz.intoto.jsonl` to verify offline
...
- Content accuracy of the study questions (use the
  [question correction template](https://github.com/ullbergm/faa-drone-trainer/issues/new?template=question-correction.yml)).
